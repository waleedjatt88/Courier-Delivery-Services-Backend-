...
const generateBookingInvoice = (parcel, customer) => {
    ...
    doc.fontSize(26).font('Helvetica-Bold').text('BOOKING INVOICE', { align: 'left' });
    doc.moveDown(0.5);
    ...
    doc.text(`Delivery Zone: ${deliveryZoneName}`, 40, itemTop + 30);
    doc.text(`Delivery Type: ${parcel.deliveryType || 'Standard'}`, 40, itemTop + 45);
    doc.text(`Booking Status: ${parcel.status}`, 40, itemTop + 60); 
    ...
    doc.moveTo(40, itemTop + 80).lineTo(555, itemTop + 80).stroke();

    const totalY = itemTop + 95;
...
const generateDeliveryInvoice = (parcel, customer) => {
    ...
    doc.fontSize(26).font('Helvetica-Bold').text('PROOF OF DELIVERY', { align: 'left' });
    doc.moveDown(0.5);
    ...
    doc.text(`Delivery Address: ${parcel.deliveryAddress}`, 40, itemTop + 15);
    doc.text(`Final Status: ${parcel.status}`, 40, itemTop + 30); 
    
    doc.text(parcel.deliveryCharge.toFixed(2), 0, itemTop, { align: 'right' });
    doc.moveTo(40, itemTop + 50).lineTo(555, itemTop + 50).stroke();
    
    const totalY = itemTop + 65;
    doc.fontSize(14).font('Helvetica-Bold').text(`Total Due: Rs. ${parcel.deliveryCharge.toFixed(2)}`, 40, totalY, { align: 'right' });
